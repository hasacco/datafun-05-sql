...
--
-- Create the `candy_targets` table using DuckDB SQL syntax and data types.
...
-- Create the `candy_products` table using DuckDB SQL syntax and data types.
...
-- Create the `candy_sales` table using DuckDB SQL syntax and data types.
...
-- In candy, targets exist independently of products and sales.
-- Therefore, load the targets table before the products and sales tables.
--
-- SQLITE ALTERNATIVE:
-- If we used SQLite, we would load data using Python and pandas.
-- Load the parent (independent) table first.
COPY candy_targets
FROM 'data/raw/candy/Candy_Targets.csv'
(HEADER, DELIMITER ',', QUOTE '"', ESCAPE '"');

-- Load the child (dependent) table second.
COPY candy_products
FROM 'data/raw/candy/Candy_Products.csv'
(HEADER, DELIMITER ',', QUOTE '"', ESCAPE '"');

COPY candy_sales
FROM 'data/raw/candy/Candy_Sales.csv'
(HEADER, DELIMITER ',', QUOTE '"', ESCAPE '"');
--
--
-- ============================================================
